...
public class AnimalController {
    ...
    public ResponseEntity<List<Animal>> listAll(
        ...
        if (especie != null) return ResponseEntity.ok(animalService.findByEspecie(especie));
        if (idadeMin != null && idadeMax != null) return ResponseEntity.ok(animalService.findByIdadeBetween(idadeMin, idadeMax));
        if (nome != null) return ResponseEntity.ok(animalService.findByNome(nome));
